...
class SimilarityClusteredStrategy(
    ...
    def _cluster_clients(
        ...
        # Generate presentation/analysis plots for this clustering round.
    ...
    def _round_results_dir(self, server_round: int) -> str:
        """Create a directory dedicated to one clustering round."""
        round_dir = os.path.join(
            self.results_dir,
            f"round_{server_round:02d}",
        )
    ...
    def _visualize_clustering(
        ...
        """
        Generate PCA and cosine-distance visualizations.

        These plots use only model-update information (Delta W).
        No patient-level data is accessed or transmitted.
        """

        if not deltas:
            logger.warning(
                "Round %d | No Delta W vectors available for visualization.",
                server_round,
            )
            return

        round_dir = self._round_results_dir(server_round)

        client_ids = list(deltas.keys())
        matrix = np.vstack([deltas[cid] for cid in client_ids])

        # These labels are only for readability in plots.
        # The full Flower client IDs remain in the CSV.
        display_labels = {
            cid: f"Client {i + 1}"
        ...
        assignments = {
            cid: self.client_clusters.get(cid, 0)
        ...
        if len(client_ids) >= 2 and matrix.shape[1] >= 2:
            try:
                ...
                ax.set_title(
                    f"Similarity Clustering of Client ΔW — Round {server_round}"
                )
                ax.set_xlabel(
                    f"Principal Component 1 ({explained[0]:.2f}% variance)"
                )
                ax.set_ylabel(
                    f"Principal Component 2 ({explained[1]:.2f}% variance)"
                )
                ax.legend()
                ...
                logger.info(
                    "Round %d | PCA clustering plot saved: %s",
                    server_round,
                    pca_path,
                )
        ...
        heatmap_path = os.path.join(
            round_dir,
            "cosine_distance_heatmap.png",
        )

        if self.last_distance_matrix is not None:
            try:
                n = len(self.last_distance_client_ids)

                fig, ax = plt.subplots(
                    figsize=(max(8, 1.2 * n), max(7, 1.0 * n))
                )
                ...
                tick_labels = [
                    display_labels.get(cid, f"Client {i + 1}")
                ...
                ax.set_title(
                    f"Cosine Distance Between Client ΔW Updates — Round {server_round}"
                )
                ...
                for i in range(n):
                    for j in range(n):
                        ax.text(
                            j,
                            i,
                            f"{self.last_distance_matrix[i, j]:.2f}",
                            ha="center",
                            va="center",
                            fontsize=8,
                        )
                ...
                logger.info(
                    "Round %d | Cosine-distance heatmap saved: %s",
                    server_round,
                    heatmap_path,
                )
                ...
                logger.exception(
                    "Round %d | Heatmap failed: %s",
                    server_round,
                    exc,
                )

        # --------------------------------------------------------------------
        # 3. ROUND-SPECIFIC CLUSTER ASSIGNMENT CSV
        # --------------------------------------------------------------------
        ...
        logger.info(
            "Round %d | Cluster assignments saved: %s",
            server_round,
            assignments_path,
        )
        ...
        for cid in client_ids:
            grouped.setdefault(
                assignments[cid],
                [],
            ).append(display_labels[cid])

        for cluster_id in sorted(grouped):
            logger.info(
                "Cluster %d: %s",
                cluster_id,
                ", ".join(grouped[cluster_id]),
            )
